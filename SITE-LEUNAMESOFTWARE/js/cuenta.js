/* ==========================================================================
   LeuName Softwares — Mi cuenta (mi-cuenta.html)
   Datos de ejemplo: cuando exista el backend de autenticación y compras,
   esta sección debe reemplazarse por los productos realmente comprados
    ...
    el.innerHTML = example.map(function (row) {
      ...
      return '<div class="owned-card">' +
        ...
        '<div class="owned-card-body">' +
          ...
          '<span class="owned-status ' + row.status + '">' + (locked ? 'Bloqueado' : 'Disponible') + '</span>' +
        '</div>' +
      '</div>';
    }).join('');
  });
